...
function send(method, command, req, callback) {
    ...
    if(self.serviceInfo && self.serviceInfo.ip && self.serviceInfo.port) {
        ...
        log.info('send to ' + this.serviceName + ' ' + this.serviceInfo.ip + ':' + this.serviceInfo.port + '/' + command);
        log.debug('send data ' + this.serviceName + '/' + command, req);
        ...
        request(options, function(err, res) {
            ...
            if(!err) {
                ...
                log.debug('response data '+ self.serviceName + '/' + command, res.data);
            }
